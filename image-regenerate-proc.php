...
if(isset($input["marginstrip"]))
  ...
  $marginstrip = 0;
if(isset($input["replaceall"])) 
	$replaceall = true;
else
	$replaceall = false;
if(isset($input["replacediff"]))
	$replacediff = true;
else
	$replacediff = false;
...
if(!$replaceall && ($task != "check_ids") && ($task != "sequential")) pecho(" Replace existing is disabled so some images may have been skipped. ");
echo '<h1>Finish</h1>';
if($task == "sequential")
...
{ global $library, $imageformats, $triplepath, $allformats, $shownotfound, $verbose,
  $indexfile, $lastimage, $task, $replaceall, $replacediff, $gdformats, $imagickformats;
  global $input, $conn;
  ...
  if($replaceall || $replacediff)  /* delete backoffice thumbnail */
...
{ global $verbose, $filter, $updatedformats, $marginstrip,$sidemargin,$tbmargin;
  ...
  global $pngorjpg, $use_webp, $replaceall, $replacediff, $imagickformats;
  try {
...
//  echo " setting:".$jpg_quality;
  ...
  { $makepng = $makewebp = $makejpg = false;
	...
	{ if($replaceall || (!file_exists($imgbase."-".$key.'.jpg')))
		$makepng = true;
	  else if($replacediff)
	  { list($fwidth, $fheight) = @getimagesize($imgbase."-".$key.'.jpg');
		if(($fwidth != $targetwidth) || ($fheight != $targetheight))
		  $makepng = true;
	  }
	}
	if(in_array("webp",$targettypes))
	{ if($replaceall || (!file_exists($imgbase."-".$key.'.webp')))
		$makewebp = true;
	  else if($replacediff)
	  { list($fwidth, $fheight) = @getimagesize($imgbase."-".$key.'.webp');
		if(($fwidth != $targetwidth) || ($fheight != $targetheight))
		  $makewebp = true;
	  }
	}
	...
	{ $img = clone $src;
	  ...
	  { $img2 = new Imagick();
	    ...
	    $img2->compositeImage($img, imagick::COMPOSITE_DEFAULT, round(($targetwidth-$newwidth)/2), round(($targetheight-$newheight)/2));
	    $img=$img2;
	  }
...
{ global $verbose, $updatedformats, $marginstrip,$sidemargin,$tbmargin;
  global $totwidth,$totheight,$totsize, $jpg_quality, $png_quality, $webp_quality;
  global $pngorjpg, $use_webp, $replaceall, $replacediff, $gdformats;
  $pos = strrpos($srcfile,".");
  $imgroot = substr($srcfile,0,$pos);
  $imgbase2 = realpath($dir.$srcfile);
  $pos = strrpos($imgbase2,".");
...
//	$img2 = imagecropauto($src,IMG_CROP_THRESHOLD,?,?);
  ...
  { $makepng = $makewebp = $makejpg = false;
	...
	{ /* If an image is too small we put a transparant border around it */
	  ...
	  imagecopyresampled($img2, $src, round(($targetwidth-$newwidth)/2), round(($targetheight-$newheight)/2), 0, 0, $newwidth, $newheight, $srcwidth, $srcheight);
