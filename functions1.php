...
/* to support multifeature the primary index of the ps_feature_product should contain three fields: id_feature, id_product and id_feature_value */
if(((defined('_TB_VERSION_') && version_compare(_TB_VERSION_ , "1.0.3", ">="))) || (version_compare(_PS_VERSION_ , "1.7.3", ">=")))
  define('MULTI_FEATURE', 1);
else
  define('MULTI_FEATURE', 0);
...
function ps_round($value, $precision, $round_mode)
{
	switch ($round_mode) {
		...
		// some shops fail on the presence of ** even when it is not executed. 
		    $precision_factor = 1;
		    if($precision >= 0)
...
{ 
  $query = "SELECT * FROM information_schema.tables
	WHERE table_schema = '".mescape($database)."' 
    AND table_name = '".mescape($table)."' LIMIT 1";
  $res = dbquery($query);
  return mysqli_num_rows($res);
}

function dbquery($query) 
{ global $conn, $mode, $verbose, $prestools_settings;
  global $prestools_settings, $prestools_settings, $logfileuser, $flog;
  $base = trim(strtoupper(substr(trim($query), 0, 6)));
  if(substr($base,0,4) == "DROP") $base = "DROP";
  if($verbose == "true")
  {	if($prestools_settings["profiling"])
  ...
  { // echo "PHP error". $e->getMessage();
    ...
    $full_error = "<p>MySQL error ".mysqli_errno($conn).": ".$error."<br>Generated by URL '".$_SERVER["PHP_SELF"]."'<br>with Query '".$query."'";
	...
	$full_error .= "<br>generated by ".$list[0]['file']." on line ".$list[0]['line']."<p>";
	if($prestools_settings["keep_log_file"] && in_array($base, ['INSERT','UPDATE','DELETE','ALTER','DROP','TRUNCA']))
    { if(!isset($flog) || (!$flog))
	  { $flog = fopen($prestools_settings["log_directory"].'/ptlog'.date("Ymd").".log","a"); 
        if(!$flog) colordie("Cannot open logfile ".$prestools_settings["log_directory"].'/ptlog'.date("Ymd").".log");
	  }
	  fwrite($flog, date("G:i:s")." ".$logfileuser." ERROR ".$full_error."\n"); 
	  //fclose($fg);
    }
    if($mode == "background") colordie($full_error);
    else colordie("</select></table>".$full_error);
  }
  if(!$res)   /* try-catch won't work with old PHP versions */
  { // echo "PHP error". $e->getMessage();
    $error = mysqli_error($conn);
    $full_error = "<p>MySQL error ".mysqli_errno($conn).": ".$error."<br>Generated by URL '".$_SERVER["PHP_SELF"]."'<br>with Query '".$query."'";
    if((mysqli_errno($conn) == "1062") && (strpos($query, "specific_price") > 0))
  ...
  { if(!isset($flog) || (!$flog))
	...
	fwrite($flog, date("G:i:s")." ".$logfileuser." ".$query."\n"); 
	//fclose($fg);
  }
  return $res;
}
...
{ global $prestools_notbought, $prestools_missing;
  ...
  "customizations"=>"TE_plugin_customizations.php","discount"=>"TE_plugin_discounts.php",
	...
	"shopz"=>"TE_plugin_shopz.php",	"translate"=>"TE_plugin_translate.php",	"virtualp"=>"TE_plugin_virtual.php" );
...
{ global $selected_img_extension, $prestools_settings, $triplepath;
  $legacy_images = get_configuration_value('PS_LEGACY_IMAGES');
  if($selected_img_extension == "")
  { if($type == "category")
    { foreach($prestools_settings["img_extensions"]  AS $ipart)
...
function get_route($routetype, $id_lang)
{ /* Thirty Bees always defines the routes. In Prestashop the values in the configuration table are not set after installation. They are set with the first save of the SEO&urls admin page. */
  $psdefaults = ['category'=>'{id}-{rewrite}', 'supplier'=>'supplier/{id}-{rewrite}', 'manufacturer'=>'brand/{id}-{rewrite}', 'cms'=>'content/{id}-{rewrite}', 'cms_category'=>'content/category/{id}-{rewrite}', 'module'=>'module/{module}{/:controller}','product'=>'{category:/}{id}-{rewrite}{-:ean13}.html'];
  if($routetype == "module")
  { $route = get_configuration_value('PS_ROUTE_module');
  }	
  else
  { if(defined('_TB_VERSION_'))  /* Thirty Bees uses configuration_lang table */
      $route = get_configuration_lang_value('PS_ROUTE_'.$routetype.'_rule',$id_lang);
    else
      $route = get_configuration_value('PS_ROUTE_'.$routetype.'_rule');
  }
  if(!$route)
	$route = $psdefaults[$routetype];
  return $route;
}
...
{ global $conn;
 ...
 {  global $triplepath,$mytables;
	...
	if(table_exists(_DB_NAME_, _DB_PREFIX_."compare_product"))
...
function layered_is_installed()
{ global $layered_installed, $triplepath;
  if(isset($layered_installed))
	  return $layered_installed;
  $res= dbquery("SHOW TABLES LIKE '"._DB_PREFIX_."layered_category'");
  if(mysqli_num_rows($res) == 0)
  { $layered_installed = false;
	return $layered_installed;
  }
  $modulename = "ps_facetedsearch";
  if (version_compare(_PS_VERSION_ , "1.7", "<"))
	  $modulename = "blocklayered";
  if(!file_exists($triplepath.'modules/'.$modulename.'/'.$modulename.'.php'))
  { $layered_installed = false;
	return $layered_installed;
  }
  $mquery = 'SELECT active, name'; 
  if(version_compare(_PS_VERSION_ , "1.6", ">="))
      $mquery .= ",enable_device";
  $mquery .= ' FROM '._DB_PREFIX_.'module m';
  $mquery .= ' LEFT JOIN '._DB_PREFIX_.'module_shop ms ON m.id_module=ms.id_module';
  $mquery .= ' WHERE name="'.$modulename.'"';
  if(version_compare(_PS_VERSION_ , "1.6", ">="))
    $mquery .= ' AND enable_device > 0';
  else
	$mquery .= ' AND active=1';
  $res = dbquery($mquery);
  if(mysqli_num_rows($res) == 0)
	...
	$layered_installed = true; 
  if(!$layered_installed) echo "NOT";
  echo " installed";
  return $layered_installed;
}
...
/* fputcsv will not put in quotes strings with a comma but without a space. */
/* The code of fputcsv3 comes from a forum where it was claimed to be the source for the PHP function. */
/* We store numeric strings longer than 11 positions like ean13 and upc as strings. 
* This will NOT prevent Excel from showing them like "1,23345E+12". However, tricks 
* that prevent this will inevitably make the csv useless for other applications.
*/
/* I have added the functionality that it will always escape strings with a semicolon or comma and NULL handling */
  function fputcsv3(&$handle, $fields = array(), $delimiter = ',', $enclosure = '"') {
    ...
    foreach ($fields as $value) {
	  if($value === NULL)
		  $value = '';
	  /* deploy quotes (enclosure) when value contains certain characters or is ean/upc/isbn */
      if (strpos($value, $delimiter) !== false ||
          ...
          strpos($value, ",") !== false ||          
		  strpos($value, ' ') !== false ||
		  (is_string($value) && ctype_digit($value) && (strlen($value)>=12))) {
        $str2 = $enclosure;
        $escaped = 0;
        $len = strlen($value);
        for ($i=0;$i<$len;$i++) {
...
{  global $prestools_settings, $headermsgs;
  ...
  <li><a href="order-edit.php">☑ Orders</a>
	...
	<li><a href="shopsearch.php">⚒ Tools &amp; Stats</a>
		<ul>
			...
			<li><a href="hook-info.php">Hook Info</a></li>
			<li><a href="override-list.php">Override List</a></li>		
			<li><a href="dbinfo.php">DbInfo</a></li>					
  ...
  if($prestools_settings["demo_mode"]) echo '<div style="background-color:#333; color:#F7F; margin-top:-12px; font-size: 18px;">This software runs in demo mode. You can access everything but changes are not saved.</div>';
  $parts = explode('/', $_SERVER["SCRIPT_NAME"]);
  if (version_compare(_PS_VERSION_ , "1.5.0.10", ">=")) /* the indexed field is added in 1.5.0.10 */
  { $query = "SELECT COUNT(DISTINCT p.id_product) AS unindexedcount FROM "._DB_PREFIX_."product p";
    $query .= " LEFT JOIN "._DB_PREFIX_."product_shop ps ON ps.id_product=p.id_product";
    $query .= " INNER JOIN "._DB_PREFIX_."shop s ON ps.id_shop=s.id_shop AND s.active=1";
    $query .= " WHERE ps.indexed='0' AND ps.visibility IN ('both', 'search') AND ps.`active` = 1";
    $res = dbquery($query);
    ...
    { echo '<div style="background-color:#ff0; color:#f00; width:100%; margin-top:-10px">';
	  ...
	  echo 'You have '.$unindexedcount.' unindexed products in your shop(s). ';
